...
El = [500,600,700,800,900,1000,1100,1200]
FC = [475, 550, 625, 700, 775, 850, 925, 1000]
...
for a in range(8):
    for b in range(8):
        ...
        ### Constant Parameters
        model.WT_Cost = Param(default=20000000)
        model.BESS_Cost = Param(default=350)
        model.El_Cost = Param(default=ElC)
        model.Comp_Cost = Param(default=40)
        model.H2_kwh = Param(default=33)
        model.FC_Cost = Param(default=FCC)
        model.Cav = Param(default=35) # $/kg
        model.WT_OM_Cost = Param(default=42000)
        model.BESS_OM_Cost = Param(default=10)
        model.El_OM_Cost = Param(default=0.02)
        model.FC_OM_Cost = Param(default=0.013)
        model.Cav_OM_Cost = Param(default = 44783)
        model.BESSdegra =Param(default = 0.05)
        model.El_effi =Param(default=0.7)
        model.FC_effi =Param(default=0.6)
        model.BESS_effi = Param(default=0.95)
        model.lifetime = Param(default=20)
        model.T_R = Param(default=6)
        ...
        model.WT = Var(within= NonNegativeIntegers)
        model.BESS =  Var(within= PositiveReals)
        model.El =  Var(within= PositiveReals)
        model.FC =  Var(within= PositiveReals)
        model.Cavern =  Var(within= PositiveReals)
        model.P_FC = Var(model.PERIOD)
        model.P_El = Var(model.PERIOD)
        model.P_Curt = Var(model.PERIOD, within= NonNegativeReals)
        model.ESSinitial = Var()
        model.ESS = Var(model.PERIOD)
        model.Cav_lvl = Var(model.PERIOD)
        model.Cavinitial = Var()
        model.Operation = Var()
